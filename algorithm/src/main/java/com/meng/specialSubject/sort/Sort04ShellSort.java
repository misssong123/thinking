 ...
 * 下面是希尔排序的基本步骤：
 *
 * 选择间隔（增量）：首先选择一个间隔（或增量）h，这个间隔通常会从数组长度的一半开始，
 * 并且每次迭代减少间隔，常见的递减规则有h=h/2，直至h=1。
 *
 * 子序列排序：对所有相隔h的元素子序列进行插入排序，即对下标为0, h, 2h, 3h... 的元素进行排序；
 * 然后对下标为1, h+1, 2h+1... 的元素进行排序；如此类推，直到所有子序列排序完成。
 *
 * 减少间隔，重复排序：将间隔h减半，然后对新的子序列进行排序，直到间隔h降为1。
 *
 * 最后一次排序：当h=1时，进行一次完整的插入排序，此时整个数组接近有序，排序效率高
 */
